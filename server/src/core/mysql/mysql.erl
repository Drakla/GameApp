...
log(Module, Line, Level, FormatFun) ->
    ...
    if
        Level =:= error ->
            util:log1(Level, Format, Arguments, Module, Line);
        true ->
...
connect(Server, PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,
   ...
   {ok, LogFun} = gen_server:call(Server, get_logfun),
    case Fun(Server, Host, Port1, User, Password, Database, LogFun,
	     Encoding, PoolId) of
	{ok, ConnPid} ->
	    Conn = new_conn(Server, PoolId, ConnPid, Reconnect, Host, Port1, User,
...
new_conn(Server, PoolId, ConnPid, Reconnect, Host, Port, User, Password, Database,
	 Encoding) ->
    case Reconnect of
	true ->
	    #conn{
...
%%   query_result()
fetch(PoolId, Query) ->
    fetch(mysql_dispatcher, PoolId, Query, 30000).
...
execute(Name, Params) when is_atom(Name), is_list(Params) ->
	...
	undefined ->
	    {error, not_in_transaction};
	State ->
	    mysql_conn:execute_local(mysql_dispatcher, State, Name, Params)
    end;
...
execute(PoolId, Name, Params, Timeout) ->
	execute(mysql_dispatcher, PoolId, Name, Params, Timeout).
...
transaction(PoolId, Fun) ->
    transaction(mysql_dispatcher, PoolId, Fun, undefined).
...
init([Server, PoolId, Host, Port, User, Password, Database, LogFun, Encoding]) ->
    ...
    case mysql_conn:start(Server, Host, Port, User, Password, Database, LogFun1,
			  Encoding, PoolId) of
	{ok, ConnPid} ->
	    Conn = new_conn(Server, PoolId, ConnPid, true, Host, Port, User, Password,
			    Database, Encoding),
...
reconnect_loop(Conn, LogFun, N) ->
    {Server, PoolId, Host, Port} = {Conn#conn.server, Conn#conn.pool_id, Conn#conn.host, Conn#conn.port},
    case connect(
         Server,
         PoolId,
		 Host,
	...
	Reason1 ->
	    ...
	    NewN = case N of
			   ...
			   ?Log2(LogFun, debug,
			       "reconnect: still unable to connect to "
			       "~p:~s:~p (~p)", [PoolId, Host, Port, Reason1]),
			   0;
		       _ ->
			   N + 1
		   end,
